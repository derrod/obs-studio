    ...
    for server in service['servers']:
        ...
        if service_type == 'ftl_output':
            try:
                ...
                print(f'Could not resolve hostname for server: {server["url"]}')
            else:
        ...
        # HLS - try connecting with requests, ignore error if < 500
            ...
            try:
                ...
                print(f'Could not connect to server: {server["url"]} (Exception: {e})')
            else:
                new_service['servers'].append(server)
        # RTMP(S) - try sending payload and check if valid handshake comes back
            ...
            try:
                    ...
                    if parsed.scheme == 'rtmps':
                        with context.wrap_socket(sock, server_hostname=hostname) as ssock:
                            ...
                            while True:
                                ...
                                # if there's some failure other than timeout, this will be empty
                                if not _tmp:
                                    break
                                recv += _tmp
                                if len(recv) >= 1536:
                ...
                print(f'Connection to server failed: {server["url"]} (Exception: {e})')
            else:
                new_service['servers'].append(server)

    if (diff := len(service['servers']) - len(new_service['servers'])) > 0:
        print(f'Removed {diff} servers from {service["name"]}')
...
if removed_something:
    ...
    with open(SERVICES_FILE, 'w', encoding='utf-8') as services_file:
        json.dump(new_services, services_file, indent=4, sort_keys=True)
    ...
    with open(PACKAGE_FILE, 'w', encoding='utf-8') as package_file:
        json.dump(package, package_file, indent=4, sort_keys=True)
