...
def post_pr_comment(s, repo, pr_num, warnings):
    formatted_warnings = '\n'.join(f'- {e.encode("utf-8")}' for e in warnings)
...
def is_valid_target(target):
    try:
        print(f'Checking if branch "{target}" is valid...')
        _ = check_output(['git', 'rev-parse', '--verify', target], stderr=STDOUT)
        return ''
...
def main():
    ...
    for target in targets:
        if err := is_valid_target(target):
            warnings.append(err)
        else:
            valid.append(target)

    # if no valid targets are specified, treat warnings as errors and exit
    if not valid:
