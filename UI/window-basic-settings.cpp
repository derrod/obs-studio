	  ...
	  ui               (new Ui::OBSBasicSettings)
{
	...
	HookWidget(ui->advOutFFCodecCompat,  CHECK_CHANGED,  OUTPUTS_CHANGED);
...
void OBSBasicSettings::ReloadCodecs(const ff_format_desc *formatDesc)
{
	...
	bool ignore_compatability = ui->advOutFFCodecCompat->isChecked();
	OBSFFCodecDesc codecDescs(ff_codec_supported(formatDesc, ignore_compatability));
...
void OBSBasicSettings::LoadAdvOutputFFmpegSettings()
{
	...
	ui->advOutFFCodecCompat->setChecked(codecCompat);
...
void OBSBasicSettings::SaveOutputSettings()
{
	...
	SaveCheckBox(ui->advOutFFCodecCompat, "AdvOut", "FFCodecCompat");
...
void OBSBasicSettings::on_advOutFFCodecCompat_stateChanged(int state)
{
	// Little hack to reload codecs when this checkbox gets checked
