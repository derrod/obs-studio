...
void OBSBasicSettings::LoadStream1Settings()
{
	...
	if (config_has_user_value(main->Config(), "Stream1", "MultitrackExtraCanvase")) {
		/* Currently we only support one canvas, so the value will just be one UUID. */
		const std::string_view uuid = config_get_string(main->Config(), "Stream1", "MultitrackExtraCanvase");
		if (!uuid.empty()) {
...
void OBSBasicSettings::SaveStream1Settings()
{
	...
	SaveComboData(ui->multitrackVideoAdditionalCanvas, "Stream1", "MultitrackExtraCanvase");
