...
// Y2023 spec
void flv_packet_ex(struct encoder_packet *packet, int32_t dts_offset,
		   uint8_t **output, size_t *size, int type)
{
	struct array_output_data data;
	struct serializer s;
	array_output_serializer_init(&s, &data);
	...
	enum video_id_t codec_id = to_video_type(codec);
...
void flv_packet_start(struct encoder_packet *packet, int32_t dts_offset,
		      uint8_t **output, size_t *size)
{
	flv_packet_ex(packet, dts_offset, output, size, PACKETTYPE_SEQ_START);
}

void flv_packet_frames(struct encoder_packet *packet, int32_t dts_offset,
		       uint8_t **output, size_t *size)
{
	const char *codec = obs_encoder_get_codec(packet->encoder);
#ifdef ENABLE_HEVC
	flv_packet_ex(packet, dts_offset, output, size,
		      (strcmp(codec, "hevc") == 0) ? PACKETTYPE_FRAMESX
						   : PACKETTYPE_FRAMES);
#else
...
void flv_packet_end(struct encoder_packet *packet, int32_t dts_offset,
		    uint8_t **output, size_t *size)
{
	flv_packet_ex(packet, dts_offset, output, size, PACKETTYPE_SEQ_END);
}

void flv_packet_metadata(obs_encoder_t *encoder, uint8_t **output, size_t *size,
			 int bits_per_raw_sample, uint8_t color_primaries,
			 int color_trc, int color_space, int min_luminance,
			 int max_luminance)
{
	// metadata array
	struct array_output_data data;
