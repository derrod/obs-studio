...
static void *mp4_replay_mux_thread(void *data)
{
	...
	if (!error) {
		muxer = mp4_mux_create(params->output, &serializer, MP4_USE_NEGATIVE_CTS, params->flavor);
...
struct obs_output_info mp4_replay_info = {
	...
	.flags = OBS_OUTPUT_AV | OBS_OUTPUT_ENCODED | OBS_OUTPUT_MULTI_TRACK_AUDIO | OBS_OUTPUT_CAN_PAUSE,
	.encoded_video_codecs = "h264;hevc;av1",
	.encoded_audio_codecs = "aac;alac;flac;opus",
