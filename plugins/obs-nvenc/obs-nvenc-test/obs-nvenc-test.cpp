...
static bool init_nvenc()
{
	...
	return nvenc->NvEncodeAPICreateInstance(&nv) == NV_ENC_SUCCESS;
}
...
static bool init_cuda()
{
	...
	return cu->cuInit(0) == CUDA_SUCCESS;
}
...
int check_thread()
{
	...
	for (int idx = 0; idx < device_infos.size(); idx++) {
