...
static bool init_encoder_base(struct nvenc_data *enc, obs_data_t *settings)
{
	...
	if (strcmp(enc->props.rate_control, "lossless") == 0) {
	...
	uint32_t gop_size = enc->props.keyint_sec
				    ? (uint32_t)enc->props.keyint_sec *
	...
	config->frameIntervalP = 1 + (int32_t)enc->props.bf;
	...
	if (strcmp(enc->props.rate_control, "CQP") == 0) {
	...
	info("settings:\n"
	     ...
	     vbr ? enc->props.target_quality : enc->props.cqp, gop_size,
	     enc->props.preset, enc->props.tune, enc->props.multipass,
...
static bool init_encoder_hevc(struct nvenc_data *enc, obs_data_t *settings)
{
	...
	/* Enable CBR padding */
	// ToDo do not disable filler data in split mode once NVIDIA fixes this
	if (config->rcParams.rateControlMode == NV_ENC_PARAMS_RC_CBR
#ifdef NVENC_12_1_OR_LATER
	    && enc->props.split_encode != NV_ENC_SPLIT_AUTO_MODE &&
	    enc->props.split_encode != NV_ENC_SPLIT_DISABLE_MODE
#endif
	) {
...
reroute:
	...
	case CODEC_H264:
		return obs_encoder_create_rerouted(encoder,
						   "obs_nvenc_h264_cuda_v2");
	case CODEC_HEVC:
		return obs_encoder_create_rerouted(encoder,
						   "obs_nvenc_hevc_cuda_v2");
	case CODEC_AV1:
		return obs_encoder_create_rerouted(encoder,
						   "obs_nvenc_av1_cuda_v2");
	}
...
void register_encoders(void)
{
	...
	if (is_codec_supported(CODEC_AV1)) {
		...
		blog(LOG_INFO, "[NVENC] AV1 is not supported");
	}
}
