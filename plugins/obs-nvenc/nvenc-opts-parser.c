...
#define APPLY_BIT_OPT(opt_name, bits)                                         \
	if (strcmp(opt->name, #opt_name) == 0) {                              \
		blog(LOG_DEBUG, "[obs-nvenc] Changing: \"%s\": %d -> %s",     \
		     #opt_name, nv_conf->opt_name, opt->value);               \
		nv_conf->opt_name =                                           \
...
static bool apply_conf_opt(const struct obs_option *opt, NV_ENC_CONFIG *nv_conf)
{
	...
	APPLY_INT_OPT(mvPrecision, NV_ENC_MV_PRECISION)
...
static bool apply_h264_opt(struct obs_option *opt, NV_ENC_CONFIG_H264 *nv_conf)
{
	...
	APPLY_INT_OPT(adaptiveTransformMode,
		      NV_ENC_H264_ADAPTIVE_TRANSFORM_MODE)
	APPLY_INT_OPT(fmoMode, NV_ENC_H264_FMO_MODE)
	APPLY_INT_OPT(bdirectMode, NV_ENC_H264_BDIRECT_MODE)
	APPLY_INT_OPT(entropyCodingMode, NV_ENC_H264_ENTROPY_CODING_MODE)
	APPLY_INT_OPT(maxNumRefFrames, uint32_t)
...
static bool apply_av1_opt(struct obs_option *opt, NV_ENC_CONFIG_AV1 *nv_conf)
{
	...
	APPLY_INT_OPT(useBFramesAsRef, NV_ENC_BFRAME_REF_MODE)
	APPLY_INT_OPT(numFwdRefs, NV_ENC_NUM_REF_FRAMES)
	APPLY_INT_OPT(numBwdRefs, NV_ENC_NUM_REF_FRAMES)
