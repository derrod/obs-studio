...
static void game_capture_destroy(void *data)
{
	...
	if (gc->audio_source) {
		obs_source_remove_active_child(gc->source, gc->audio_source);
		obs_source_release(gc->audio_source);
	}
...
static void game_capture_update(void *data, obs_data_t *settings)
{
	...
	setup_audio_source(gc->source, &gc->audio_source, settings,
...
static void game_capture_save(void *data, obs_data_t *settings)
{
	struct game_capture *gc = data;
	if (!gc->audio_source)
		return;

	obs_data_t *audio_data = obs_save_source(gc->audio_source);
...
static bool game_capture_audio_render(void *data, uint64_t *ts_out,
				      ...
				      size_t sample_rate)
{
	...
	return render_child_audio(gc->audio_source, ts_out, output, mixers,
				  channels);
}
