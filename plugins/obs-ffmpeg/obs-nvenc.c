...
/* These are missing from the ffnvcodec headers for some reason */
static const int CU_AD_FORMAT_NV12 = 0xb0;
				 ...
				 struct nv_cuda_surface *nvsurf)
{
	...
	if (!enc->fallback) {
		...
		case NV_ENC_BUFFER_FORMAT_NV12:
			// ToDo Figure out why NV12 does not work
			// desc.Format = CU_AD_FORMAT_NV12;
			desc.Format = CU_AD_FORMAT_UNSIGNED_INT8;
			// Additional half-size plane for UV data
				    ...
				    struct nv_cuda_surface *surf)
{
	...
	if (enc->surface_format == NV_ENC_BUFFER_FORMAT_NV12) {
		...
		// ToDo figure out why NV12 doesn't work
		// NV12 CUDA surfaces have two planes for Y and UV, we need to copy both
		/*
		CUarray nv12_uv;
		CU_FAILED(cu->cuArrayGetPlane(&nv12_uv, surf->tex, 1))
		m.srcPitch = frame->linesize[1];
		m.srcHost = frame->data[1];
		m.dstArray = nv12_uv;
		m.Height /= 2;
