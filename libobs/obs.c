...
static void obs_free_data(void)
{
	...
	FREE_OBS_HASH_TABLE(hh, &data->sources, source);
	FREE_OBS_HASH_TABLE(hh_uuid, &data->sources_uuid, source);
...
void obs_enum_all_sources(bool (*enum_proc)(void *, obs_source_t *),
			  void *param)
{
	obs_enum_uuid(&obs->data.sources_uuid, &obs->data.sources_mutex,
		      enum_proc, param);
}
					...
					void *(*addref)(void *))
{
	...
	/* If context head has a hash table use that for the lookup */
...
{
	return get_context_by_name(&obs->data.sources, name,
...
{
	return get_context_by_uuid(&obs->data.sources_uuid, uuid,
				   &obs->data.sources_mutex,
				   obs_source_addref_safe_);
}
...
obs_source_t *obs_get_transition_by_name(const char *name)
{
	...
	pthread_mutex_lock(&obs->data.sources_mutex);
	HASH_ITER (hh_uuid, (struct obs_context_data *)obs->data.sources_uuid,
		   context, tmp) {
		source = (struct obs_source *)context;
...
void obs_reset_source_uuids()
{
	...
	struct obs_context_data *ht =
		(struct obs_context_data *)obs->data.sources_uuid;
	...
	obs->data.sources_uuid = (struct obs_source *)new_ht;
