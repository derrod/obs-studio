...
static bool obs_init_gpu_conversion(struct obs_core_video_mix *video)
{
	...
	video->using_gbra_tex = info->format == VIDEO_FORMAT_BGRA ? true : false; // FIXME
	...
	if (video->using_gbra_tex)
		blog(LOG_INFO, "GBRA texture support enabled");
	else
		blog(LOG_INFO, "GBRA texture support not available");
	...
	} else if (video->using_gbra_tex) {
		video->convert_textures_encode[0] = gs_texture_create(info->width, info->height, GS_GBRA, 1, NULL,
