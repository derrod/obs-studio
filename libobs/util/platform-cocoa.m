...
bool os_keychain_save(const char *key, const char *data)
{
    ...
    NSDictionary *insert = @ {
        ...
        (__bridge id) kSecAttrService: @"OBS Studio",
    ...
    if (status == errSecDuplicateItem) {
        ...
        if (status != errSecSuccess) {
            blog(LOG_ERROR, "Keychain item \"%s\" could not be updated: %d", key, status);
        }
    } else if (status != errSecSuccess) {
        blog(LOG_ERROR, "Keychain item \"%s\" could not be saved: %d", key, status);
    }
...
bool os_keychain_load(const char *key, char **data)
{
    ...
    if (status != errSecSuccess || !result) {
        blog(LOG_ERROR, "Keychain item \"%s\" could not be read: %d", key, status);
        return false;
    }
...
bool os_keychain_delete(const char *key)
{
    ...
    if (status != errSecSuccess && status != errSecItemNotFound) {
        blog(LOG_WARNING, "Keychain item \"%s\" could not be deleted: %d", key, status);
        return false;
    }
