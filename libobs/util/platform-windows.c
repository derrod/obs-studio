...
bool os_keychain_save(const char *key, const char *data)
{
	...
	struct dstr uuid_str = {0};
	dstr_printf(&uuid_str, "OBS|%s", key);
	...
	if (!success) {
		blog(LOG_ERROR, "Keychain item \"%s\" could not be saved: %d",
		     key, GetLastError());
	}
...
bool os_keychain_load(const char *key, char **data)
{
	...
	if (success) {
		*data = bstrdup_n((const char *)cred->CredentialBlob,
		...
		blog(LOG_ERROR, "Keychain item \"%s\" could not be read: %d",
		     key, GetLastError());
	}
...
bool os_keychain_delete(const char *key)
{
	...
	if (!success) {
		...
		if (err == ERROR_NOT_FOUND) {
			...
			blog(LOG_WARNING,
			     "Keychain item \"%s\" could not be deleted: %d",
			     key, err);
		}
	}
